...
export class SelectConstructor {
  constructor(data = null, props) {
    ...
    if (this.config.init) {
      ...
      const selectItems = data ? (typeof data === 'string' ? document.querySelectorAll(data) : [data]) : document.querySelectorAll('[data-custom-select]');
  ...
  getSelectedOptionsData(originalSelect, type) {
    ...
    return {
      ...
      values: selectedOptions.length ? selectedOptions.filter(option => option.value).map(option => option.value) : ['No options'],
      html: selectedOptions.length ? selectedOptions.map(option => this.getSelectElementContent(option)) : ['No options']
    }
  }
  ...
  getOption(selectOption, originalSelect) {
    ...
    let dataset = selectOption.dataset;
    let datasetHTML = ``;
    if (Object.keys(dataset).length) {
      Object.keys(dataset).forEach(key=>{
        datasetHTML += `data-${key}="${dataset[key]}" `;
      })
    }
    ...
    if (originalSelect.multiple) { // Если мультивыбор
      ...
      if (!originalSelect.hasAttribute('data-show-selected')) {
        ...
        if (selectItem.querySelector(`.${this.selectClasses.classSelectOptionSelected}`)) {
          selectItem.querySelector(`.${this.selectClasses.classSelectOptionSelected}`).classList.remove(this.selectClasses.classSelectOptionSelected);
        }
        // Выделяем классом элемент
